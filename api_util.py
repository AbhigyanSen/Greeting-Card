...
def generate_birthday_card(df,ngs):
...
def get_generated_card(user_ngs,Event,num_images,df):
  # PROMPT -----------------------------------------------------------------------------------------------
  ...
  prompt = generate_birthday_card(df,user_ngs)
...
# IMAGE GENERATION -------------------------------------------------------------------------------------
  ...
  if 0 <= num_images <= 6:
      for i in range(num_images):
          ...
          text = f"{Event} {Text}! Cheers to celebrating another year filled with your awesomeness!"
          
          # Wishing you a wonderful [Birthday/Anniversary]! 
  
          # Generate the speech from the text
          tts = gTTS(text)
          audio_file = "/home/dcsadmin/Documents/PersonalisedGreeting/GeneratedImages/output_audio.mp3"
          tts.save(audio_file)
  
          # Load the image
          image_path = f"/home/dcsadmin/Documents/PersonalisedGreeting/GeneratedImages/{user_ngs}Card{i+1}.png"
          image = Image.open(image_path)
  
          # Save the image as a temporary file compatible with moviepy
          # temp_image_path = "temp_image.png"
          # image.save(temp_image_path)
  
          # Load the image and audio into moviepy
          image_clip = ImageClip(image_path, duration=5)  # Duration can be adjusted
          ...
          output_video_path = f"/home/dcsadmin/Documents/PersonalisedGreeting/GeneratedImages/{user_ngs}Card{i+1}.mp4"
  ...
  desired_images = [f"{user_ngs}Card{i+1}.mp4" for i in range(num_images)]
  for i in range(num_images):
    desired_images.append(f"{user_ngs}Card{i+1}.png")
